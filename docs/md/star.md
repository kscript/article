[Developer-Zero-To-Mastery](https://github.com/wx-chevalier/Developer-Zero-To-Mastery)  
一个程序员的技术视野. star过来主要为了了解其它程序员的知识面, 作者开了很多坑没填, 广度有了, 但深度有限, 以后有时间再看吧  

[git-recipes](https://github.com/geeeeeeeeek/git-recipes)  
一个git的基础手册

[JavaGuide](https://github.com/Snailclimb/JavaGuide)  
对Java接触的不多, star过来深入学习下

[DeepLearning-500-questions](https://github.com/scutan90/DeepLearning-500-questions)  
[deeplearningbook-chinese](https://github.com/exacity/deeplearningbook-chinese)  
深度学习. 未来可能属于人工智能  

[react-redux](https://github.com/reduxjs/react-redux)  
react的状态管理  

[omi](https://github.com/Tencent/omi)  
基于 Web Components 设计的前端统一框架, 热度不是很高, 保持关注  


[PIE](https://github.com/lojjic/PIE)  
用来给一些不支持css3的浏览器做hack  

[i18next](https://github.com/i18next/i18next)  
一个国际化解决方案

[javascript-algorithms](https://github.com/trekhleb/javascript-algorithms)  
JavaScript算法与数据结构

[jQuery-Waterwheel-Carousel](https://github.com/bkosborne/jQuery-Waterwheel-Carousel)  
一个基于jQuery的3D轮播插件

[book](https://github.com/GuoYongfeng/book)  
一本介绍前端技术的书. 涉及: git/node/es6/webpack/css预处理/react

[typescript-starter](https://github.com/bitjson/typescript-starter)  
快速开始一个基于typescript的 nodeJS/javascript 项目  

[jsdoc-vuejs](https://github.com/Kocal/jsdoc-vuejs)  
[jsdoc-vuedoc](https://github.com/ccqgithub/jsdoc-vuedoc)  
vue项目的注释文档生成工具

[Typedoc-Webpack-Plugin](https://github.com/Microsoft/Typedoc-Webpack-Plugin)  
一个typescript注释文档生成工具的Webpack扩展

[tsdoc](https://github.com/Microsoft/tsdoc)  
一个typescript注释文档生成工具  

[vue-property-decorator](https://github.com/kaorun343/vue-property-decorator)  
在vue文件写ts时用到的属性装饰器

[TypeScript](https://github.com/Microsoft/TypeScript)  
一个javascript类型的超集, 用于代码的静态类型检查

[jsdoc](https://github.com/jsdoc3/jsdoc)  
注释文档生成工具

[easy-mock](https://github.com/easy-mock/easy-mock)  
一个快速生成模拟数据的持久化服务

[redux](https://github.com/reduxjs/redux)  
状态管理

[php-sdk](https://github.com/qiniu/php-sdk)  
七牛云的php sdk, 用于上传个人网站的静态资源

[json-server](https://github.com/typicode/json-server)  
创建mock服务, 用于模拟请求. 在我自己开发的ks-mock模块里用到了这个

[Mock](https://github.com/nuysoft/Mock)  
模拟数据生成. 可与json-server搭配使用

[taro](https://github.com/NervJS/taro)  
一个多端统一开发框架, 使用react编写一次代码, 生成能运行在微信/百度/支付宝/字节跳动小程序、H5、React Native 等的应用。

[taro-ui](https://github.com/NervJS/taro-ui)  
taro的ui库

[vue-design](https://github.com/HcySunYang/vue-design)  
vue源码分析

[mpvue](https://github.com/Meituan-Dianping/mpvue)  
基于 Vue.js 的小程序开发框架

[mpvue-weui](https://github.com/KuangPF/mpvue-weui)  
基于mpvue的一套ui

[CharacterMap](https://github.com/mathew-kurian/CharacterMap)  
一个字体信息查看工具

[English-level-up-tips-for-Chinese](https://github.com/byoungd/English-level-up-tips-for-Chinese)  
英语学习

[muse-ui](https://github.com/museui/muse-ui)  
[vuetify](https://github.com/vuetifyjs/vuetify)  
基于 vue 2.0 的 material ui

[vant](https://github.com/youzan/vant)  
[mint-ui](https://github.com/ElemeFE/mint-ui)  
基于Vue移动端ui

[webpack-chain](https://github.com/neutrinojs/webpack-chain)  
使用 webpack-chain 来配置webpack

[NIM_Web_Demo](https://github.com/netease-im/NIM_Web_Demo)  
网易云信IM 的 Web 版 Demo

[egret-docs](https://github.com/egret-labs/egret-docs)  
白鹭引擎使用文档

[vscode-vue-snippets](https://github.com/hollowtree/vscode-vue-snippets)  
vscode编辑器vue代码片段

[ws](https://github.com/websockets/ws)  
一个用于操作 node.js websocket 的库

[slim](https://github.com/slim-template/slim)  
一种模板语言, 告别写html时复杂的标签结构

[Proton](https://github.com/a-jie/Proton)  
粒子引擎

[slidePage](https://github.com/lipten/slidePage)  
一个h5滚屏插件

[ding-custom-home](https://github.com/beanLau/ding-custom-home)  
钉钉微应用脚手架

[v-charts](https://github.com/ElemeFE/v-charts)  
基于 Vue2.0 和 ECharts 封装的图表组件

[webpack](https://github.com/webpack/webpack)  
javascript 模块打包器

[react](https://github.com/facebook/react)  
前端框架(库)

[freeCodeCamp](https://github.com/freeCodeCamp/freeCodeCamp)  
代码训练营

[Tween](https://github.com/zhangxinxu/Tween)  
一个缓动函数库

[Good-Text-Share](https://github.com/Wscats/Good-Text-Share)  
收集了一些前端方面的文章

[react-markdown-editor](https://github.com/jrm2k6/react-markdown-editor)  
基于react的markdown编辑器

[vonic](https://github.com/wangdahoo/vonic)  
一个移动端的vue ui, 已停止更新, 可做参考, 切勿使用, 真是个坑

[vue-codemirror](https://github.com/surmon-china/vue-codemirror)  
一个vue版的代码编辑器

[CodeMirror](https://github.com/codemirror/CodeMirror)  
代码编辑器

[atom](https://github.com/atom/atom)  
atom编辑器

[deno](https://github.com/denoland/deno)  
nodejs之父新开的一个副本

[vue-jstree](https://github.com/zdy1988/vue-jstree)  
vue 树形结构

[curlconverter](https://github.com/NickCarneiro/curlconverter)  
将curl命令转换为 python, javascript, php, R, Go, Rust, JSON

[Python-100-Days](https://github.com/jackfrued/Python-100-Days)  
Python入门

[libweibo](https://github.com/xiaosier/libweibo)  
新浪微博开放平台php sdk

[electron-vue](https://github.com/SimulatedGREG/electron-vue)  
一套用于桌面应用开发的 electron + vue 脚手架

[deformation](https://github.com/PUGE/deformation)  
vue 可拖动/调整尺寸 区域

[AlloyImage](https://github.com/AlloyTeam/AlloyImage)  
基于HTML5的专业级图像处理开源引擎

[ant-design](https://github.com/ant-design/ant-design)  
企(彩)业(蛋)级ui设计体系

[hosts](https://github.com/googlehosts/hosts)  
hosts文件

[webpack](https://github.com/vuejs-templates/webpack)  
vue 脚手架 模板

[browser-compat-data](https://github.com/mdn/browser-compat-data)  
mdn 文档

[node-tutorial](https://github.com/wangdoc/node-tutorial)  
nodejs 教程

[vue-timepickr](https://github.com/ispal/vue-timepickr)  
vue 时间选择器

[VueCnodeJS](https://github.com/shuiRong/VueCnodeJS)  
vue 项目 demo

[notebook](https://github.com/zhenyong/notebook)  
一个程序员的笔记本

[v8](https://github.com/v8/v8)  
chrome V8引擎

[vue](https://github.com/vuejs/vue)  
一个渐进式前端框架

[d3](https://github.com/d3/d3)  
数据可视化函数库

[javascript](https://github.com/airbnb/javascript)  
javascript代码风格指南

[font-carrier](https://github.com/purplebamboo/font-carrier)  
一个字体操作库

[vscode-background](https://github.com/shalldie/vscode-background)  
vscode背景图

[VS-Code-Extension-Doc-ZH](https://github.com/Liiked/VS-Code-Extension-Doc-ZH)  
vscode 插件开发 中文文档

[jekyll](https://github.com/jekyll/jekyll)   
一个博客生成工具

[chinese-independent-developer](https://github.com/1c7/chinese-independent-developer)   
中国独立开发者项目列表  

[RSSHub](https://github.com/DIYgod/RSSHub)   
一个 RSS 生成器

[vscode](https://github.com/Microsoft/vscode)   
vscode编辑器

[vscode-npm](https://github.com/fknop/vscode-npm)   
用于管理NPM命令的vscode扩展

[vscode-extension-samples](https://github.com/Microsoft/vscode-extension-samples)   
一个简单的vscode插件案例

[webpack2-lessons](https://github.com/kingvid-chan/webpack2-lessons)   
一个前端程序员的webpack学习之路

[esprima](https://github.com/jquery/esprima)   
一个ECMAScript解析器

[estree](https://github.com/estree/estree)   
一种json风格的AST规范

[jscodeshift](https://github.com/facebook/jscodeshift)   
一种基于抽象语法树（AST）的代码自动化重构工具。

[how-javascript-works](https://github.com/Troland/how-javascript-works)   
JavaScript的工作原理

[console-importer](https://github.com/pd4d10/console-importer)   
chrome浏览器扩展, 用于在Chrome控制台轻松导入JS和CSS资源

[verdaccio](https://github.com/verdaccio/verdaccio)   
搭建 npm 私库

[pm2](https://github.com/Unitech/pm2)   
nodejs 进程管理工具

[pm2-explorer](https://github.com/alsiola/pm2-explorer)   
vscode 插件, 用于在vscode中使用pm2管理进程

[miniblink49](https://github.com/weolar/miniblink49)   
一个小巧、轻量的浏览器内核

[pwa](https://github.com/vuejs-templates/pwa)   
使用vue脚手架创建的, 基于webpack模板的pwa模板

[sprite](https://github.com/iwangx/sprite)   
css雪碧图简单制作工具, 这个工具是c#的, 在这里打个广告, 我自己写的[js版的](https://github.com/kscript/css-sprite-generator)

[nedb](https://github.com/louischatriot/nedb)   
一个javascript数据库, 可以用于 nodejs, nw.js, electron, 浏览器环境  

[Nano-SQL](https://github.com/ClickSimply/Nano-SQL)   
一个客户端, 服务端通用的数据库

[chinese-poetry](https://github.com/chinese-poetry/chinese-poetry)   
诗词

[TSW](https://github.com/Tencent/TSW)   
日志监控

[element](https://github.com/ElemeFE/element)   
饿了么 基于 vue 2.0 的 ui库

[framework7](https://github.com/framework7io/framework7)   
移动端 material 风格的组件库

[framework7-plugin-3d-panels](https://github.com/framework7io/framework7-plugin-3d-panels)   
framework7 扩展 3D面板

[vue-easy-gantt](https://github.com/mamboer/vue-easy-gantt)   
vue 日程表插件

[fonteditor-core](https://github.com/kekee000/fonteditor-core)   
字体编辑器核心功能

[fonteditor](https://github.com/ecomfe/fonteditor)   
百度字体编辑器

[architecture.taobao-alibaba](https://github.com/davideuler/architecture.taobao-alibaba)   
互联网公司架构

[three.js](https://github.com/mrdoob/three.js)   
3D 库

[socket.io](https://github.com/socketio/socket.io)   
WebSocket操作库

[express](https://github.com/expressjs/express)   
nodejs 框架

[30-seconds-of-code](https://github.com/30-seconds/30-seconds-of-code)   
可以在30秒内理解的.. 有用的javascript片段

[Front-End-Checklist](https://github.com/thedaviddias/Front-End-Checklist)   
前端开发清单

[babel](https://github.com/babel/babel)   
使用balel转换你编写的下一代的代码

[hexo](https://github.com/hexojs/hexo)   
一个博客生成工具

[koa](https://github.com/koajs/koa)   
nodejs 框架

[video.js](https://github.com/videojs/video.js)   
html5 & flash 视频播放器

[vue-quill-editor](https://github.com/surmon-china/vue-quill-editor)   
vue版的富文本编辑器

[awesome-wechat-weapp](https://github.com/justjavac/awesome-wechat-weapp)   
微信小程序开发资源汇总

[pug](https://github.com/pugjs/pug)   
一种模板语言引擎

[layui](https://github.com/sentsin/layui)   
前端 ui 库

[fks](https://github.com/JacksonTian/fks)   
前端技能汇总

[NeteaseCloudMusicApi](https://github.com/Binaryify/NeteaseCloudMusicApi)   
网易云音乐 api

[WebFrontEndStack](https://github.com/unruledboy/WebFrontEndStack)   
web前端技术栈

[hexo-theme-even](https://github.com/ahonn/hexo-theme-even)   
一个hexo主题

[uni-app](https://github.com/dcloudio/uni-app)   
使用 Vue.js 开发跨平台应用的前端框架

[next](https://github.com/alibaba-fusion/next)   
基于React的可配置组件库。

[incubator-echarts](https://github.com/apache/incubator-echarts)   
可视化交互式图表 echarts

[js-sdk](https://github.com/qiniu/js-sdk)   
七牛 JavaScript SDK

[gitbook](https://github.com/GitbookIO/gitbook)   
一个基于 Node.js 的命令行工具,可使用 Github/Git 和 Markdown 来制作精美的电子书

[js-yaml](https://github.com/nodeca/js-yaml)   
YAML(一种简洁的非标记语言) 解析器

[swig](https://github.com/paularmstrong/swig)   
一个模板引擎

[Valine](https://github.com/xCss/Valine)   
一款基于Leancloud的快速、简洁且高效的无后端评论系统

[composer](https://github.com/composer/composer)   
PHP的依赖关系管理器

[material-kit](https://github.com/creativetimofficial/material-kit)   
一套 material 风格的ui

[next.js](https://github.com/zeit/next.js)   
一个轻量级的 React 服务端渲染应用框架。

[pdf.js](https://github.com/mozilla/pdf.js)   
使用HTML5构建的pdf阅读器

[clipboard.js](https://github.com/zenorocha/clipboard.js)   
html5剪切板

[quill](https://github.com/quilljs/quill)   
富文本编辑器

[standard](https://github.com/standard/standard)   
javascript风格指南

[vue-cli](https://github.com/vuejs/vue-cli)   
vue 项目脚手架

[nuxt.js](https://github.com/nuxt/nuxt.js)   
一个基于 Vue.js 的轻量级应用框架, 可用于SSR

[jquery-pjax](https://github.com/defunkt/jquery-pjax)   
pushState + ajax 构建无刷新应用

[wepy](https://github.com/Tencent/wepy)   
一个小程序组件化开发框架

[flux](https://github.com/facebook/flux)   
Facebook用于构建客户端Web应用程序的一个系统架构

[vue-devtools](https://github.com/vuejs/vue-devtools)   
一个调试vue项目的浏览器插件

[create-react-native-app](https://github.com/react-community/create-react-native-app)   
react native应用脚手架

[ivideo](https://github.com/phobal/ivideo)   
一个可以观看国内主流视频平台所有视频的客户端

[lib-flexible](https://github.com/amfe/lib-flexible)   
可伸缩布局方案

[UglifyJS](https://github.com/mishoo/UglifyJS)   
解析/压缩/美化 javascript 的nodejs工具

[vConsole](https://github.com/Tencent/vConsole)   
移动端调试工具

[Swipe](https://github.com/thebird/Swipe)   
轮播插件

[es5-shim](https://github.com/es-shims/es5-shim)   
在不支持es5的浏览器使用es5的垫片

[qrcodejs](https://github.com/davidshimjs/qrcodejs)   
二维码生成工具

[chrome-app-samples](https://github.com/GoogleChrome/chrome-app-samples)   
一些 chrome 应用

[front-end-collect](https://github.com/foru17/front-end-collect)   
一些前端开发相关的优秀网站、博客、以及活跃开发者

[font-spider](https://github.com/aui/font-spider)   
字蛛, 抓取页面中用到指定字体的文字, 生成一个字体包

[f2etest](https://github.com/alibaba/f2etest)   
一个面向前端、测试、产品等岗位的多浏览器兼容性测试整体解决方案。

[curl](https://github.com/cujojs/curl)   
一种小型、快速、可扩展的模块加载程序，可处理AMD、CommonJS模块/1.1、CSS、HTML/text和遗留脚本。

[source-map](https://github.com/mozilla/source-map)   
生成源码映射

[skilltree](https://github.com/phodal/skilltree)   
前端技能树 demo

[bodyparser](https://github.com/koajs/bodyparser)   
koa 内容解析器

[china_regions](https://github.com/wecatch/china_regions)   
省市区数据

[xss-listener](https://github.com/cagataycali/xss-listener)   
xss渗透工具

[httphijack](https://github.com/chokcoco/httphijack)   
使用Javascript实现前端防御http劫持及防御XSS攻击，并且对可疑攻击进行上报

[oculus-nodejs-threejs-example](https://github.com/phodal/oculus-nodejs-threejs-example)   
Oculus + Node.js + Three.js 打造VR世界

[shelljs](https://github.com/shelljs/shelljs)   
在nodejs中使用shell命令

[typescript-book-chinese](https://github.com/jkchao/typescript-book-chinese)   
深入理解 TypeScript 中文版

[node](https://github.com/nodejs/node)   
一个基于 Chrome V8 引擎的 JavaScript 运行环境。

[axios](https://github.com/axios/axios)   
一个通用的http请求模块

[jquery](https://github.com/jquery/jquery)   
一个javascript库

[free-programming-books-zh_CN](https://github.com/justjavac/free-programming-books-zh_CN)   
免费的计算机编程类中文书籍

[material-ui](https://github.com/mui-org/material-ui)   
material 风格的ui

[yarn](https://github.com/yarnpkg/yarn)   
一个包管理器

[storybook](https://github.com/storybooks/storybook)   
UI 组件管理

[anime](https://github.com/juliangarnier/anime)   
一个javascript动画引擎

[todomvc](https://github.com/tastejs/todomvc)   
帮助你选择一个MV*框架

[swiper](https://github.com/nolimits4web/swiper)   
移动端滑动轮播

[material](https://github.com/angular/material)   
AngularJS 版 material 风格的ui

[octotree](https://github.com/ovity/octotree)   
github 代码树插件

[highlight.js](https://github.com/highlightjs/highlight.js)   
代码高亮

[egg](https://github.com/eggjs/egg)   
基于 koa 的一个nodejs框架

[js-xss](https://github.com/leizongmin/js-xss)   
根据白名单过滤 HTML(防止 XSS 攻击)

[AR.js](https://github.com/jeromeetienne/AR.js)   
用于手机端AR

[cypress](https://github.com/cypress-io/cypress)   
前端E2E测试框架

[animate.css](https://github.com/daneden/animate.css)   
一个跨浏览器的CSS动画库。

[zhubangbang-javascript-notes](https://github.com/zhubangbang/zhubangbang-javascript-notes)   
一个前端程序员的javaScript学习笔记

[blog](https://github.com/yygmind/blog)   
[Blog](https://github.com/mqyqingfeng/Blog)   
前端程序员博客

[33-js-concepts](https://github.com/stephentian/33-js-concepts)   
每个 JavaScript 工程师都应懂的33个概念

[gold-miner](https://github.com/xitu/gold-miner)   
掘金翻译计划

[analyse-vue-cli](https://github.com/lxchuan12/analyse-vue-cli)   
分析vue-cli@2.9.3 搭建的webpack项目工程

[mobx](https://github.com/mobxjs/mobx)   
简单、可扩展的状态管理。

[nodejs-learning-guide](https://github.com/chyingp/nodejs-learning-guide)   
Nodejs学习笔记以及经验总结

[fe9-library](https://github.com/frontend9/fe9-library)   
九部知识库

[autoprefixer](https://github.com/postcss/autoprefixer)   
css 自动前缀

[postcss](https://github.com/postcss/postcss)   
一个转换样式的JS插件

[pure](https://github.com/pure-css/pure)   
一组轻量级、响应式纯css模块

[front-end-guide](https://github.com/grab/front-end-guide)   
前端开发指南

[You-Dont-Need-JavaScript](https://github.com/you-dont-need/You-Dont-Need-JavaScript)   
CSS很强大，没有JS你也可以做很多事情

[Front-End-Performance-Checklist](https://github.com/thedaviddias/Front-End-Performance-Checklist)   
前端性能清单

[stylus](https://github.com/stylus/stylus)   
一个CSS预处理器

[uncss](https://github.com/uncss/uncss)   
从CSS中删除未使用的样式

[Web-Series](https://github.com/wx-chevalier/Web-Series)   
web开发文档

[fetool](https://github.com/nieweidong/fetool)   
前端工具集

[blog](https://github.com/cssmagic/blog)   
CSS魔法 - 博客

[Web](https://github.com/qianguyihao/Web)   
Web前端入门和进阶学习笔记

[blog](https://github.com/creeperyang/blog)   
前端博客，关注基础知识和性能优化。

[cc](https://github.com/ccforward/cc)   
代码库 & Blog

[canvas-special](https://github.com/bxm0927/canvas-special)   
经典 canvas 实例

[You-need-to-know-css](https://github.com/l-hammer/You-need-to-know-css)   
css技巧

[css-handbook](https://github.com/doyoe/css-handbook)   
CSS参考手册

[Guide](https://github.com/Aaaaaashu/Guide)   
前端开发指南

[knowledge](https://github.com/f2e-awesome/knowledge)   
前端技术架构图谱

[blog](https://github.com/youngwind/blog)   
梁少峰的个人博客

[Front-end-tutorial](https://github.com/windiest/Front-end-tutorial)   

[frontendDaily](https://github.com/kujian/frontendDaily)   
前端开发技术日报

[cssgrace](https://github.com/cssdream/cssgrace)   
一个由 PostCSS 驱动，面向未来的 CSS 后处理工具。

[50weekly](https://github.com/ihtml5/50weekly)   
收集高质量的前端资源

[blog](https://github.com/JChehe/blog)   
一个前端程序员的文章与翻译

[node-interview](https://github.com/ElemeFE/node-interview)   
如何通过饿了么 Node.js 面试

[blockly](https://github.com/google/blockly)   
基于Web的可视化编程编辑器。

[pkuseg-python](https://github.com/lancopku/pkuseg-python)   
多领域中文分词工具

[FE-interview](https://github.com/qiu-deqing/FE-interview)   
前端面试题和答案

[imagemin](https://github.com/imagemin/imagemin)   
图片压缩

[jenkins](https://github.com/jenkinsci/jenkins)   
自动化服务器

[electron](https://github.com/electron/electron)   
桌面应用开发

[source](https://github.com/file-icons/source)   
文件字体图标

[seti-ui](https://github.com/jesseweed/seti-ui)   
atom 深色主题

[bootstrap](https://github.com/twbs/bootstrap)   
css ui 框架

[cnpmjs.org](https://github.com/cnpm/cnpmjs.org)   
npm包管理器的淘宝镜像

[node-in-debugging](https://github.com/nswbmw/node-in-debugging)   
Node.js 调试指南

[node-api-cn](https://github.com/nodejscn/node-api-cn)   
Node.js API 中文文档

[puppeteer-deep](https://github.com/zhentaoo/puppeteer-deep)   
node 爬虫

[node-v8](https://github.com/nodejs/node-v8)   
一个自动更新的node.js实验版本, 使用已知的良好的v8

[Ghost](https://github.com/TryGhost/Ghost)   
基于 Node.js 构建的开源博客平台

[html2canvas](https://github.com/niklasvh/html2canvas)   
html 转 canvas

[js-xlsx](https://github.com/SheetJS/js-xlsx)   
使用js操作xlsx

[N-blog](https://github.com/nswbmw/N-blog)   
一起学 Node.js

[uppy](https://github.com/transloadit/uppy)   
一个模块化的文件上传工具

[svg.js](https://github.com/svgdotjs/svg.js)   
用于操作和设置SVG动画的轻量级库

[Awsome-Front-End-learning-resource](https://github.com/helloqingfeng/Awsome-Front-End-learning-resource)   
前端资源汇总

[jszip](https://github.com/Stuk/jszip)   
使用javascript创建、读取和编辑.zip文件

[vscode-extension-vscode](https://github.com/Microsoft/vscode-extension-vscode)   
给vscode插件开发者提供的vscode模块

[webaudio-examples](https://github.com/mdn/webaudio-examples)   
web 音频 api 使用示例

[git-history](https://github.com/pomber/git-history)   
vscode插件 从Git存储库快速浏览文件的历史记录

[web-socket-js](https://github.com/gimite/web-socket-js)   
有flash支持的HTML5 WebSocket

[ice](https://github.com/alibaba/ice)   
通过 GUI 工具极速构建中后台应用

[shields](https://github.com/badges/shields)   
徽章

[pkg](https://github.com/zeit/pkg)   
将你的nodejs应用打包成一个可执行程序(exe)

[Windows-universal-samples](https://github.com/Microsoft/Windows-universal-samples)   
Windows平台的API示例

[ndm](https://github.com/npm/ndm)   
直接使用npm包部署服务?

[sack.vfs](https://github.com/d3x0r/sack.vfs)   

[mapbox-gl-js](https://github.com/mapbox/mapbox-gl-js)   
Mapbox GL JS是一个JavaScript库，用于Web上交互式、可自定义的矢量图。

[chromedriver](https://github.com/electron/chromedriver)   
用于下载 Electron 使用的 chromedriver

[node-gui](https://github.com/yue/node-gui)   
node 图形界面
[yue](https://github.com/yue/yue)   
一个创建跨平台gui应用的库

[velocity](https://github.com/julianshapiro/velocity)   
一个动画引擎

[puppeteer](https://github.com/GoogleChrome/puppeteer)   
nodejs 爬虫

[fullPage.js](https://github.com/alvarotrigo/fullPage.js)   
整页切换

[git-tips](https://github.com/521xueweihan/git-tips)   
Git的奇技淫巧

[flv.js](https://github.com/bilibili/flv.js)   
HTML5 flv 播放器

[draggable](https://github.com/Shopify/draggable)   
js拖动

[Snap.svg](https://github.com/adobe-webplatform/Snap.svg)   
一个操作svg的javascript库

[nodemailer](https://github.com/nodemailer/nodemailer)   
使用nodejs发送邮件

[popper.js](https://github.com/FezVrasta/popper.js)   
一个弹出层插件

[fine-uploader](https://github.com/FineUploader/fine-uploader)   
一个多文件上传/图片预览剪切

[debug](https://github.com/visionmedia/debug)   
一个测试工具, 使用nodecore开发

[apidoc](https://github.com/apidoc/apidoc)   
RESTful api文档生成器

[workbox](https://github.com/GoogleChrome/workbox)   
PWA JavaScript库

[medis](https://github.com/luin/medis)   
Redis Mac数据库管理应用。

[emotion](https://github.com/emotion-js/emotion)   
CSS-in-JS

[markdown-it](https://github.com/markdown-it/markdown-it)   
markdown 解析器

[tween.js](https://github.com/tweenjs/tween.js)   
一款可生成平滑动画效果的js动画库

[Snap.js](https://github.com/jakiestfu/Snap.js)   
Facebook风格侧边菜单效果

[chroma.js](https://github.com/gka/chroma.js)   
颜色操作库

[nodeppt](https://github.com/ksky521/nodeppt)   
web演示工具 node ppt

[turn.js](https://github.com/blasten/turn.js)   
html5 翻页效果

[hls.js](https://github.com/video-dev/hls.js)   
JavaScript HLS客户端

[cropperjs](https://github.com/fengyuanchen/cropperjs)   
图片裁剪

[How-To-Ask-Questions-The-Smart-Way](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way)   
如何提问

[devtool](https://github.com/Jam3/devtool)   
通过chromium devtools运行node.js程序

[jsdiff](https://github.com/kpdecker/jsdiff)   
文本差异比较

[FileAPI](https://github.com/mailru/FileAPI)   
一组用于处理文件的JavaScript工具。多重上传、拖放和分块文件上传。图像：按exif裁剪、调整大小和自动定位

[github](https://github.com/github-tools/github)   
对 github api 进行了一次包装

[simpread](https://github.com/Kenshin/simpread)   
一个阅读器应用

[pwa](https://github.com/lukeed/pwa)   
一个 pwa 脚手架

[typescript-tutorial](https://github.com/xcatliu/typescript-tutorial)   
TypeScript 入门教程

[image-diff](https://github.com/uber-archive/image-diff)   
图片差异比较

[AlloyTouch](https://github.com/AlloyTeam/AlloyTouch)   
触摸运动方案

[2048-AI](https://github.com/ovolve/2048-AI)   
一个简单的AI应用

[console.image](https://github.com/adriancooney/console.image)   
在控制台显示图片

[shake.js](https://github.com/alexgibson/shake.js)   
一个自定义的“震动”事件插件

[distpicker](https://github.com/fengyuanchen/distpicker)   
三级联动

[js-stack-from-scratch](https://github.com/verekia/js-stack-from-scratch)   
从零开始的JS学习栈

[webpack.js.org](https://github.com/docschina/webpack.js.org)   
webpack 中文文档

[WebGLBook](https://github.com/tparisi/WebGLBook)   
WebGL 案例

[graphiql](https://github.com/graphql/graphiql)   
一种api查询语言

[awesome-python-cn](https://github.com/jobbole/awesome-python-cn)   
Python资源大全

[jsPDF](https://github.com/MrRio/jsPDF)   

[reactjs101](https://github.com/kdchang/reactjs101)   
从零开始学习 ReactJS

[exploring-es6](https://github.com/es6-org/exploring-es6)   
探索 ES6 中文版

[mostly-adequate-guide-chinese](https://github.com/llh911001/mostly-adequate-guide-chinese)   
JS 函数式编程指南中文版

[hex](https://github.com/netease-youdao/hex)   
使用html5和node.js构建桌面应用程序的解决方案

[chromium](https://github.com/chromium/chromium)   
chromium github 官方镜像

[jslinux-deobfuscated](https://github.com/levskaya/jslinux-deobfuscated)   
用js实现的一个linux系统

[verlet-js](https://github.com/subprotocol/verlet-js)   
一个用js写的物理引擎

[julia](https://github.com/JuliaLang/julia)   
一种新生代的编程语言

[flutter-go](https://github.com/alibaba/flutter-go)   
flutter demo 演示与中文文档

[flutter](https://github.com/flutter/flutter)   
一个移动UI框架，可以快速在iOS和Android上构建高质量的原生用户界面

[es6tutorial](https://github.com/ruanyf/es6tutorial)   
ECMAScript 6入门

[Examples](https://github.com/plane000/Examples)   
多语言代码示例供参考

[cn.vuejs.org](https://github.com/vuejs/cn.vuejs.org)   
vue中文文档

[weekly](https://github.com/dt-fe/weekly)   
前端精读周刊

[danger-js](https://github.com/danger/danger-js)   
在代码审查中发现危险的代码

[unified](https://github.com/unifiedjs/unified)   
语法数转换工具

[TIL](https://github.com/xiaohesong/TIL)   
一些学习资源

[sweetalert2](https://github.com/sweetalert2/sweetalert2)   
一个美化alert弹窗的插件

[chameleon](https://github.com/didi/chameleon)   
变色龙 一套代码运行多端

[bootstrap-notify](https://github.com/mouse0270/bootstrap-notify)   
bootstrap 通知插件

[caniuse](https://github.com/Fyrd/caniuse)   
兼容性

[docs](https://github.com/webpack/docs)   
webpack 文档

[vue-markdown](https://github.com/miaolz123/vue-markdown)   
vue 版的markdown 编辑预览组件

[rollup](https://github.com/rollup/rollup)   
在node里使用es6模块规范

[thinscript](https://github.com/evanw/thinscript)   
一种实验性的编程语言，可编译为javascript、WebAssembly和C

[black-hole.js](https://github.com/cliffcrosland/black-hole.js)   
使用WebGL制作的黑洞效果

[vue-class-component](https://github.com/vuejs/vue-class-component)   
带有ES / TypeScript 装饰器风格的Vue组件
 